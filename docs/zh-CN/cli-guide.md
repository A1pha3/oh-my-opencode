# Oh-My-OpenCode 命令行工具指南

本文档提供 Oh-My-OpenCode 命令行工具的完整使用指南。

---

## 1. 概述

Oh-My-OpenCode 提供可通过 `bunx oh-my-opencode` 命令访问的命令行工具。CLI 支持多种功能，包括插件安装、环境诊断和会话执行。
...
用于初始 Oh-My-OpenCode 设置的交互式安装工具。基于 `@clack/prompts` 提供美观的 TUI（文本用户界面）。
...
1. **提供商选择**：从 Claude、ChatGPT 或 Gemini 中选择 AI 提供商
...
| 选项 | 描述 |
|------|------|
| `--no-tui` | 在非交互模式下运行，不显示 TUI（适用于 CI/CD 环境） |
| `--verbose` | 显示详细日志 |
...
| 选项 | 描述 |
|------|------|
| `--enforce-completion` | 保持会话活动，直到所有 TODO 完成 |
...
| `--client-id <id>` | OAuth 客户端 ID（如果服务器支持动态客户端注册，则为可选） |
| `--scopes <scopes>` | 逗号分隔的 OAuth 范围 |

### 令牌存储

令牌存储在 `~/.config/opencode/mcp-oauth.json` 中，权限为 `0600`（仅所有者读写）。密钥格式：`{serverHost}/{resource}`。

---

## 7. `auth` - 身份验证管理

管理 Google Antigravity OAuth 身份验证。使用 Gemini 模型需要此功能。
...
CLI 在以下位置搜索配置文件（按优先级排序）：

1. **项目级**：`.opencode/oh-my-opencode.json`
